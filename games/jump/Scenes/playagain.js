...
export default class playAgain extends Phaser.Scene {
    ...
    create() {
        ...
        timer = this.time.addEvent({
            delay: 1000, // 1 second
            callback: this.updateTimer,  // 🔥 Use this.updateTimer
            callbackScope: this,       
            loop: true
        });
    ...
    updateTimer() {
        if (countDown > 0) {
            countDown--;
            this.timerText.setText(this.formatTime(countDown)); // Update the displayed time
        } else {
            timer.remove(); // Stop the timer when it reaches zero
            this.timerText.setText("Time's up!"); // Display message when time is up
        }
    }
